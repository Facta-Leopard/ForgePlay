...
typedef struct FPFrameGenerationDisplayCandidate
{
    ...
    bool pairedCurrent;
    /* Legacy ABI field name. For an explicitly captured-source state only,
     * this is its capture content timestamp, never physical presentation. */
...
typedef struct FPFrameGenerationTelemetry
{
    ...
    double currentQueueOldestAge;
    double currentOutputMaximumAge;
    char state[16];
    char reason[64];
} FPFrameGenerationTelemetry;

/* Constant-time scheduler state. Render/capture callbacks must not calculate
 * rolling cadence diagnostics just to inspect ownership or queue capacity. */
typedef struct FPFrameGenerationProgress
{
    bool captureDemandActive;
    bool outputActive;
    uint32_t presentationsInFlight;
    uint32_t presentationReceiptsPending;
    uint64_t midpointPresented;
...
/* Explicit GPU-capture content clock for sources without a public physical
 * presentation receipt (WineD3D OpenGL). After the GPU fence signals, pass
 * the capture submission content timestamp. These observations only order content
 * and decide interpolation demand; they NEVER populate source presentation
 * counters/cadence. Output receipts retain the same strict Metal contract. */
...
/* The native capture/presentation join supplies its current monotonic time
 * immediately before publishing the ready output. Source presentation remains
 * the content timeline; readyTime measures only time spent awaiting output
 * service, excluding capture-completion latency. Unknown clocks use zero. */
...
/* The production callback supplies its monotonic decision time. An unsubmitted
 * midpoint yields to its matching Current once that Current has waited ready
 * for one effective service slot. Accepted originals and submitted pairs retain
 * their existing order. The legacy untimed entry point does not infer a clock. */
