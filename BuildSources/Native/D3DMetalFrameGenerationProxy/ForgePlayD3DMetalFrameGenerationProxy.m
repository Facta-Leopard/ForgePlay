...
typedef struct FPFrameGenerationRuntimeCounters
{
    ...
    uint64_t displayCallbackCount;
    uint64_t displayCallbackLateCount;
    double displayCallbackLateMaximumMilliseconds;
    double displaySubmitCPUMaximumMilliseconds;
    double sourceGPUMillisecondsTotal;
    uint64_t sourceGPUSamples;
    double displayGPUMillisecondsTotal;
    uint64_t displayGPUSamples;
...
)
{
    ...
    length = snprintf(
        ...
        "\tmidpoint_dropped_timeline_reversal=%llu"
        ...
        /* Parser-compatible legacy key for output Current cadence. HUD Original
         * uses source_cadence_hz; keep this telemetry field unchanged. */
        ...
        "\tcurrent_queue_depth=%u\tcurrent_queue_oldest_age_ms=%.3f"
        "\tcurrent_output_max_age_ms=%.3f"
        "\tmidpoint_dropped_current_age=%llu"
        "\tdisplay_callback_count=%llu\tdisplay_callback_late_count=%llu"
        "\tdisplay_callback_late_max_ms=%.3f"
        "\tdisplay_submit_cpu_max_ms=%.3f"
        "\tsource_gpu_ms_total=%.3f\tsource_gpu_samples=%llu"
        "\tdisplay_gpu_ms_total=%.3f\tdisplay_gpu_samples=%llu"
        "\texecutable_sha256=%s\n",
        (long)getpid(),
        telemetry.state,
        ...
        (unsigned long long)telemetry.midpointDroppedTimelineReversal,
        ...
        telemetry.currentQueueOldestAge * 1000.0,
        telemetry.currentOutputMaximumAge * 1000.0,
        (unsigned long long)telemetry.midpointDroppedCurrentAge,
        (unsigned long long)runtimeCounters.displayCallbackCount,
        (unsigned long long)runtimeCounters.displayCallbackLateCount,
        runtimeCounters.displayCallbackLateMaximumMilliseconds,
        runtimeCounters.displaySubmitCPUMaximumMilliseconds,
        runtimeCounters.sourceGPUMillisecondsTotal,
        (unsigned long long)runtimeCounters.sourceGPUSamples,
        runtimeCounters.displayGPUMillisecondsTotal,
        (unsigned long long)runtimeCounters.displayGPUSamples,
...
static dispatch_queue_t FPTelemetryQueue(void)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        queue = dispatch_queue_create("com.forgeplay.frame-generation.telemetry", attributes);
    });
    return queue;
}

static void FPEnqueueTelemetryRecord(
    FPFrameGenerationTelemetry telemetry,
    FPFrameGenerationRuntimeCounters runtimeCounters
)
{
    /* One record for the entire process, even across short-lived sessions.
     * A stalled file cannot accumulate snapshots or retain a Metal session.
     * The queued block owns values only, never a session, drawable, or handle. */
    static atomic_bool pending = false;
    if (atomic_exchange(&pending, true)) return;
    dispatch_async(FPTelemetryQueue(), ^{
        FPWriteTelemetryRecord(telemetry, runtimeCounters);
        atomic_store(&pending, false);
    });
}

static double FPCommandBufferGPUDurationMilliseconds(id<MTLCommandBuffer> commandBuffer)
{
    if (commandBuffer.status != MTLCommandBufferStatusCompleted) return 0.0;
...
)
{
    ...
    /* Error reporting is best effort, including during immediate process exit.
     * A blocked destination must not hold a renderer or grow pending work. */
    FPEnqueueTelemetryRecord(telemetry, runtimeCounters);
}
...
static NSString *FPFrameCheckString(FPFrameGenerationTelemetry telemetry, BOOL holdsPreviousOutput)
{
    BOOL sourceOwnsOutput = !telemetry.outputActive && !holdsPreviousOutput &&
        (strcmp(telemetry.state, "monitoring") == 0 ||
         strcmp(telemetry.state, "priming") == 0);
    NSString *suffix = strcmp(telemetry.state, "priming") == 0 ? @" · Priming" : @"";
    if (!telemetry.sourcePresentAccepted && !telemetry.outputActive)
        return [NSString stringWithFormat:
            @"Target      %6u Hz%@\n"
             "Original        -- FPS\n"
             "Generated      0.0 FPS\n"
             "Displayed       -- FPS", telemetry.targetFrameRate, suffix];
    return [NSString stringWithFormat:
        @"Target      %6u Hz%@\n"
         "Original    %6.1f FPS\n"
         "Generated   %6.1f FPS\n"
         "Displayed   %6.1f FPS",
        telemetry.targetFrameRate, suffix,
        telemetry.sourceCadenceHz,
        sourceOwnsOutput ? 0.0 : telemetry.generatedCadenceHz,
        sourceOwnsOutput ? telemetry.sourceCadenceHz : telemetry.finalCadenceHz];
}

@interface FPFrameGenerationMetalResources : NSObject
@property(nonatomic, strong) id<MTLDevice> device;
@property(nonatomic, strong) id<MTLCommandQueue> displayQueue;
@property(nonatomic, strong) id<MTLLibrary> library;
@property(nonatomic, strong) NSMutableDictionary<NSNumber *, id<MTLRenderPipelineState>> *pipelines;
@end
@implementation FPFrameGenerationMetalResources
@end

static NSNumber *FPCompositionPipelineKey(MTLPixelFormat format, BOOL currentOnly)
{
    return @(((uint64_t)format << 1) | (currentOnly ? 1u : 0u));
}

static dispatch_queue_t FPMetalPreparationQueue(void)
{
    static dispatch_queue_t queue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("com.forgeplay.frame-generation.metal-prepare",
            dispatch_queue_attr_make_with_qos_class(DISPATCH_QUEUE_SERIAL, QOS_CLASS_UTILITY, 0));
    });
    return queue;
}

static void FPExposeSourceDuringSurfacePreparation(
    CAMetalDisplayLink *displayLink,
    CAMetalLayer *outputLayer,
    CAMetalLayer *retainedOutputLayer
)
{
    displayLink.paused = YES;
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
    outputLayer.hidden = YES;
    retainedOutputLayer.hidden = YES;
    [CATransaction commit];
}

/* Metal compilation receives values only. It never reads a view/layer or
 * updates session state, so optional shader preparation cannot stall Wine's
 * main executor. Existing same-device resources are reusable across resize. */
static FPFrameGenerationMetalResources *FPPrepareMetalResources(
    id<MTLDevice> device,
    MTLPixelFormat format,
    id<MTLCommandQueue> queue,
    id<MTLLibrary> library,
    NSDictionary<NSNumber *, id<MTLRenderPipelineState>> *pipelines
)
{
    if (!device || format == MTLPixelFormatInvalid) return nil;
    FPFrameGenerationMetalResources *resources = [FPFrameGenerationMetalResources new];
    resources.device = device;
    resources.displayQueue = queue ? queue : [device newCommandQueueWithMaxCommandBufferCount:
        FP_FRAME_GENERATION_MAX_PRESENTATIONS_IN_FLIGHT];
    NSError *error = nil;
    resources.library = library ? library : [device
        newLibraryWithSource:FPFrameGenerationCompositionShaderSource()
        options:nil error:&error];
    resources.pipelines = pipelines ? [pipelines mutableCopy] : [NSMutableDictionary dictionary];
    if (!resources.displayQueue || !resources.library) return nil;
    for (NSUInteger currentOnly = 0; currentOnly <= 1; ++currentOnly)
    {
        NSNumber *key = FPCompositionPipelineKey(format, currentOnly != 0);
        if (resources.pipelines[key]) continue;
        id<MTLFunction> vertex = [resources.library newFunctionWithName:@"fp_frame_generation_vertex"];
        id<MTLFunction> fragment = [resources.library newFunctionWithName:
            currentOnly ? @"fp_frame_generation_current" : @"fp_frame_generation_midpoint"];
        ...
        if (pipeline) resources.pipelines[key] = pipeline;
    }
    return resources.pipelines[FPCompositionPipelineKey(format, YES)] ? resources : nil;
}
...
- (void)updateFrameCheck;
- (void)failWithReason:(const char *)reason;
- (void)failWithReason:(const char *)reason
    expectedSurfaceGeneration:(uint64_t)expectedSurfaceGeneration;
- (BOOL)prepareDemandMetalResourcesOnMain;
- (void)prepareOutputSurfaceOnMainResettingState:(BOOL)resetState;
- (void)finishOutputSurfaceOnMainResettingState:(BOOL)resetState;
...
- (BOOL)recordCaptureBusyLockedAtTime:(CFTimeInterval)now
                            telemetry:(FPFrameGenerationProgress)telemetry;
...
- (void)scheduleCaptureResourceRecoveryForSurfaceGeneration:(uint64_t)surfaceGeneration;
...
static BOOL FPInstallCommandBufferHooksForObject(
    id<MTLCommandBuffer> commandBuffer
);
...
)
{
    ...
    for (uint32_t index = 0; index < sessionCount; ++index)
    {
        ...
        if (!bound)
        {
            ...
            if (bound && ensureCommandBufferHooks &&
...
)
{
    ...
    Original original = (Original)FPOriginalMetalImplementation(self, selector);
    (void)FPBindTrackedSourceDrawable((id<CAMetalDrawable>)drawable, self, NO);
...
)
{
         ...
         ++index)
    {
        ...
        if (observed == classIdentity) return YES;
        if (observed) continue;
        uintptr_t empty = 0;
    ...
    /* Exhausting the class fast-path cache does not undo successful hooks. */
    return YES;
}
...
@implementation FPD3DMetalFrameGenerationSession
{
    ...
    atomic_bool _telemetryWritePending;
    ...
    uint64_t _displayCallbackCount;
    uint64_t _displayCallbackLateCount;
    double _displayCallbackLateMaximumMilliseconds;
    double _displaySubmitCPUMaximumMilliseconds;
    double _sourceGPUMillisecondsTotal;
    uint64_t _sourceGPUSamples;
    double _displayGPUMillisecondsTotal;
    uint64_t _displayGPUSamples;
...
{
    if (error) *error = nil;
    return _compositionPipelines[FPCompositionPipelineKey(pixelFormat, currentOnly)];
}

- (BOOL)prepareDemandMetalResourcesOnMain
{
    NSAssert(NSThread.isMainThread, @"%@", @"demand resource inspection is main-thread bound");
    id<MTLDevice> device = _sourceLayer.device ? _sourceLayer.device : _device;
    return _resourceDevice == device && _displayQueue && _compositionLibrary &&
        _compositionPipelines[FPCompositionPipelineKey(_sourceLayer.pixelFormat, YES)] != nil;
}

- (void)prepareOutputSurfaceOnMainResettingState:(BOOL)resetState
{
    NSAssert(NSThread.isMainThread, @"%@", @"surface preparation is main-thread bound");
    if (atomic_load(&_invalidated) || atomic_load(&_failed))
    {
        atomic_store(&_surfaceUpdatePending, false);
        return;
    }
    os_unfair_lock_lock(&_stateLock);
    BOOL demandActive = [self callbackStateIsUsableLocked] &&
        FPFrameGenerationGetProgress(_state).captureDemandActive;
    ...
    {
        [self finishOutputSurfaceOnMainResettingState:resetState];
        return;
    }
    if (resetState)
    {
        // A changed device/format retires the old surface's FIFO, just as the
        // synchronous surface reset did. Do it before optional compilation so
        // a last generated image cannot cover the still-running source.
        atomic_store(&_sourceCaptureEnabled, false);
        FPExposeSourceDuringSurfacePreparation(_displayLink, _outputLayer, _retainedOutputLayer);
        atomic_store(&_displayLinkPausedForWork, true);
        os_unfair_lock_lock(&_stateLock);
        if ([self callbackStateIsUsableLocked])
        {
            FPFrameGenerationResetForSurfaceChange(_state, "surface-resource-preparation");
            [self resetTextureBookkeepingLocked];
            _lastFrameCheckUpdateTime = 0.0;
        }
        os_unfair_lock_unlock(&_stateLock);
        [self updateFrameCheck];
        resetState = NO;
    }
    id<MTLDevice> device = _sourceLayer.device ? _sourceLayer.device : _device;
    MTLPixelFormat format = _sourceLayer.pixelFormat;
    BOOL sameDevice = _resourceDevice == device;
    id<MTLCommandQueue> queue = sameDevice ? _displayQueue : nil;
    id<MTLLibrary> library = sameDevice ? _compositionLibrary : nil;
    NSDictionary *pipelines = sameDevice ? [_compositionPipelines copy] : nil;
    atomic_store(&_sourceCaptureEnabled, false);
    __weak FPD3DMetalFrameGenerationSession *weakSelf = self;
    dispatch_async(FPMetalPreparationQueue(), ^{
        @autoreleasepool {
            FPFrameGenerationMetalResources *resources = FPPrepareMetalResources(
                device, format, queue, library, pipelines
            );
            dispatch_async(dispatch_get_main_queue(), ^{
                FPD3DMetalFrameGenerationSession *strongSelf = weakSelf;
                if (!strongSelf) return;
                if (atomic_load(&strongSelf->_invalidated) || atomic_load(&strongSelf->_failed))
                {
                    atomic_store(&strongSelf->_surfaceUpdatePending, false);
                    return;
                }
                id<MTLDevice> currentDevice = strongSelf->_sourceLayer.device ? strongSelf->_sourceLayer.device : strongSelf->_device;
                if (currentDevice != device || strongSelf->_sourceLayer.pixelFormat != format)
                {
                    // One pending preparation owns this gate. A changed surface
                    // starts the next preparation only after this one retires.
                    [strongSelf prepareOutputSurfaceOnMainResettingState:resetState];
                    return;
                }
                if (!resources)
                {
                    atomic_store(&strongSelf->_surfaceUpdatePending, false);
                    [strongSelf failWithReason:"metal-resource-initialization-failed"];
                    return;
                }
                BOOL reusesObservedQueue = strongSelf->_displayQueue == resources.displayQueue;
                [strongSelf releaseDemandMetalResourcesOnMain];
                strongSelf->_resourceDevice = resources.device;
                strongSelf->_displayQueue = resources.displayQueue;
                strongSelf->_compositionLibrary = resources.library;
                strongSelf->_compositionPipelines = resources.pipelines;
                // Both queues previously came from this identical public factory
                // with a count of three. Observe the queue actually used for output;
                // source capture is encoded on the renderer's own command buffer.
                // A reused queue may already have three writers in flight;
                // never allocate a probe command buffer on that busy queue.
                if (!reusesObservedQueue)
                    (void)FPInstallCommandQueueObservationHooks(strongSelf->_displayQueue);
                [strongSelf finishOutputSurfaceOnMainResettingState:resetState];
            });
        }
    });
}
                     ...
                     failureReasonCapacity:(size_t)failureReasonCapacity
{
    ...
    atomic_init(&_telemetryWritePending, false);
    ...
    if (_frameCheckEnabled)
    {
        ...
        frameCheckLayer.string = FPFrameCheckString(
            FPFrameGenerationTelemetrySnapshotAtTime(_state, CACurrentMediaTime()), NO
        );
    }
...
- (void)scheduleCaptureResourceRecoveryForSurfaceGeneration:(uint64_t)surfaceGeneration
{
    if (atomic_load(&_invalidated) || atomic_load(&_failed) ||
        ...
        return;
    os_unfair_lock_lock(&_stateLock);
    BOOL currentCapture = [self callbackStateIsUsableLocked] &&
        surfaceGeneration == _surfaceGeneration &&
        !atomic_load(&_surfaceUpdatePending);
    if (currentCapture) atomic_store(&_sourceCaptureEnabled, false);
    os_unfair_lock_unlock(&_stateLock);
    if (!currentCapture)
    {
        atomic_store(&_captureRecoveryPending, false);
        return;
    }
    /* Stop admitting source textures before retiring stalled capture state. The
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        BOOL superseded = surfaceGeneration != self->_surfaceGeneration ||
            atomic_load(&self->_surfaceUpdatePending);
        os_unfair_lock_unlock(&self->_stateLock);
        if (superseded)
        {
            // A surface transition already owns the new capture pool. Never
            // let a delayed recovery wipe its captures or rearm old resources.
            atomic_store(&self->_captureRecoveryPending, false);
            return;
        }
        ...
        if (!device || drawableSize.width <= 0.0 ||
...
{
    if (atomic_load(&_invalidated) || atomic_load(&_failed) ||
        atomic_exchange(&_surfaceUpdatePending, true)) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        [self prepareOutputSurfaceOnMainResettingState:NO];
    });
}

- (void)finishOutputSurfaceOnMainResettingState:(BOOL)resetState
{
    NSAssert(NSThread.isMainThread, @"%@", @"surface publication is main-thread bound");
    if (atomic_load(&_invalidated) || atomic_load(&_failed))
    {
        atomic_store(&_surfaceUpdatePending, false);
        return;
    ...
    [self rebuildOutputSurfaceResettingState:resetState];
    BOOL outputPrepared = _outputLayer != nil && _displayLink != nil;
    os_unfair_lock_lock(&_stateLock);
    outputPrepared = outputPrepared && [self callbackStateIsUsableLocked] &&
        FPFrameGenerationGetProgress(_state).captureDemandActive &&
        _captureTextures.count == FP_CAPTURE_TEXTURE_POOL_CAPACITY;
    os_unfair_lock_unlock(&_stateLock);
    if (outputPrepared && !atomic_load(&_invalidated) && !atomic_load(&_failed))
    {
        // Source drawables remain direct until every optional resource exists.
        _sourceLayer.framebufferOnly = NO;
        atomic_store(&_sourceCaptureEnabled, true);
    }
    else if (!atomic_load(&_failed))
    {
...
- (void)rebuildOutputSurfaceResettingState:(BOOL)resetState
{
    ...
    if (_outputLayer)
    {
        ...
        if ([self callbackStateIsUsableLocked])
            currentOutputWasActive =
                FPFrameGenerationGetProgress(_state).outputActive;
        os_unfair_lock_unlock(&_stateLock);
    }
    ...
    /* Request the user's selected cadence in every range field. Offering N/2
     * as our minimum lets the output callback itself become a half-rate limit
     * even when the source is faster. This remains a best-effort request:
     * Core Animation and the display retain actual service cadence control. */
...
- (void)sourceSurfaceConfigurationDidChange
{
    ...
    dispatch_block_t update = ^{
        if (!atomic_load(&self->_invalidated) &&
            !atomic_load(&self->_failed))
        {
            [self prepareOutputSurfaceOnMainResettingState:YES];
            return;
        }
    ...
    (id<MTLCommandBuffer>)commandBuffer
{
    ...
    BOOL observedSourceCommandBuffer = NO;
    for (uint32_t index = 0;
         index < FP_SOURCE_DRAWABLE_TICKET_CAPACITY;
         ++index)
    {
        if (![self prepareNextSourceCaptureBeforeCommit:commandBuffer])
            break;
        observedSourceCommandBuffer = YES;
    }
    if (!observedSourceCommandBuffer) return;
    /* One sample per source command buffer/session, not per drawable ticket.
     * This duration includes the renderer's work and any appended captures;
     * it is not a measurement of the blit alone. */
    __weak FPD3DMetalFrameGenerationSession *weakSelf = self;
    [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> completed) {
        double duration = FPCommandBufferGPUDurationMilliseconds(completed);
        FPD3DMetalFrameGenerationSession *strongSelf = weakSelf;
        if (!strongSelf || duration <= 0.0) return;
        os_unfair_lock_lock(&strongSelf->_stateLock);
        if ([strongSelf callbackStateIsUsableLocked])
        {
            strongSelf->_sourceGPUMillisecondsTotal += duration;
            ++strongSelf->_sourceGPUSamples;
        }
    ...
    (id<MTLCommandBuffer>)commandBuffer
{
    ...
    if ([self callbackStateIsUsableLocked])
    {
             ...
             ++index)
        {
            ...
            sourceTexture =
                (__bridge id<MTLTexture>)ticket->sourceTextureIdentity;
            FPFrameGenerationProgress telemetry =
                FPFrameGenerationGetProgress(_state);
        ...
        !sourceTexture || !capturedTexture)
    {
        if (shouldScheduleCaptureRecovery)
            [self scheduleCaptureResourceRecoveryForSurfaceGeneration:surfaceGeneration];
        return YES;
    }
                                               ...
                                               status:(MTLCommandBufferStatus)status
{
    if (!ticketID) return;
    BOOL shouldAcceptCapture = NO;
    BOOL shouldFailSourceCapture = NO;
    uint64_t sourceSequence = 0;
    ...
    if ([self callbackStateIsUsableLocked] && ticketIndex >= 0)
    {
        ...
        if (surfaceGeneration == _surfaceGeneration &&
            ticket->surfaceGeneration == surfaceGeneration &&
            ticket->captureSlot == slot && ticket->join.captureEncoded)
        {
            ...
            if (disposition != FPFrameGenerationSourceCaptureJoinDuplicate)
            {
                shouldFailSourceCapture = status == MTLCommandBufferStatusError;
                if (!succeeded)
                {
    ...
    if (!matchedTicket)
        (void)[self retireCaptureCompletionLockedForSlot:slot
                                      surfaceGeneration:surfaceGeneration];
    os_unfair_lock_unlock(&_stateLock);
    if (shouldFailSourceCapture)
    {
        [self failWithReason:"source-capture-command-failed"
          ...
          surfaceGeneration:(uint64_t)surfaceGeneration
{
    ...
    plan = FPFrameGenerationRecordCaptureReadyAtTime(
        _state,
        sourceSequence,
        presentedTime,
        CACurrentMediaTime()
...
{
    CFTimeInterval callbackEntryTime = CACurrentMediaTime();
    ...
    CFTimeInterval callbackReadyTime = CACurrentMediaTime();
    CFTimeInterval callbackDeadline = update.targetTimestamp;
    if (isfinite(callbackDeadline) && callbackDeadline > 0.0 &&
        callbackReadyTime >= callbackDeadline)
    {
        ++_displayCallbackLateCount;
        _displayCallbackLateMaximumMilliseconds = fmax(
            _displayCallbackLateMaximumMilliseconds,
            (callbackReadyTime - callbackDeadline) * 1000.0
        );
    ...
    candidate = FPFrameGenerationAcquireDisplayCandidateAtTime(
        _state, callbackReadyTime
    );
    if (candidate.kind == FPFrameGenerationOutputNone)
    {
        FPFrameGenerationProgress waitingTelemetry =
            FPFrameGenerationGetProgress(_state);
        if (_readyPairEpoch &&
          ...
          candidate.epoch == _readyPairEpoch))
    {
        ...
        FPFrameGenerationProgress acquiredTelemetry =
            FPFrameGenerationGetProgress(_state);
    ...
    [drawable addPresentedHandler:^(id<MTLDrawable> presentedDrawable) {
        ...
        FPFrameGenerationProgress beforePresentation = {0};
        FPFrameGenerationProgress telemetry = {0};
        ...
        beforePresentation =
            FPFrameGenerationGetProgress(strongSelf->_state);
        ...
        if (result.joinRetired)
            (void)[strongSelf removePresentationReceiptLockedForCandidate:
                candidate
                surfaceGeneration:surfaceGeneration];
        telemetry = FPFrameGenerationGetProgress(strongSelf->_state);
    ...
    [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> completed) {
        ...
        double gpuDuration = FPCommandBufferGPUDurationMilliseconds(completed);
        ...
        if ([strongSelf callbackStateIsUsableLocked])
        {
            ...
            if (result.matched && result.writerRetired)
            {
                if (gpuDuration > 0.0)
                {
                    strongSelf->_displayGPUMillisecondsTotal += gpuDuration;
                    ++strongSelf->_displayGPUSamples;
                }
                NSInteger receiptIndex = [strongSelf
                    presentationReceiptIndexLockedForCandidate:candidate
                    surfaceGeneration:surfaceGeneration];
    ...
    [drawable present];
    CFTimeInterval submittedTime = CACurrentMediaTime();
    os_unfair_lock_lock(&_stateLock);
    if ([self callbackStateIsUsableLocked] && submittedTime >= callbackEntryTime)
        _displaySubmitCPUMaximumMilliseconds = fmax(
            _displaySubmitCPUMaximumMilliseconds,
            (submittedTime - callbackEntryTime) * 1000.0
        );
    os_unfair_lock_unlock(&_stateLock);
    [self schedulePresentationWatchdog];
}
...
- (void)retireHeldOutputAfterActivation
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([self callbackStateIsUsableLocked])
            outputActive =
                FPFrameGenerationGetProgress(self->_state).outputActive;
        os_unfair_lock_unlock(&self->_stateLock);
        if (!outputActive)
        {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
            ...
            strongSelf->_hostView.layer)
        {
            ...
            if (validSnapshot)
            {
                ...
                if (layer) layer.string = FPFrameCheckString(
                    currentTelemetry, strongSelf->_retainedOutputLayer != nil &&
                        !strongSelf->_retainedOutputLayer.hidden
                );
            }
        }
...
- (void)emitTelemetryForced:(BOOL)forced
{
    ...
    /* Collapse simultaneous snapshot requests without blocking their callers.
     * The process-wide writer separately admits at most one value record. */
    if (atomic_exchange(&_telemetryWritePending, true)) return;
    if (atomic_load(&_invalidated))
    {
        atomic_store(&_telemetryWritePending, false);
        return;
    }
    ...
    runtimeCounters.displayCallbackCount = _displayCallbackCount;
    runtimeCounters.displayCallbackLateCount = _displayCallbackLateCount;
    runtimeCounters.displayCallbackLateMaximumMilliseconds =
        _displayCallbackLateMaximumMilliseconds;
    runtimeCounters.displaySubmitCPUMaximumMilliseconds =
        _displaySubmitCPUMaximumMilliseconds;
    runtimeCounters.sourceGPUMillisecondsTotal = _sourceGPUMillisecondsTotal;
    runtimeCounters.sourceGPUSamples = _sourceGPUSamples;
    runtimeCounters.displayGPUMillisecondsTotal = _displayGPUMillisecondsTotal;
    runtimeCounters.displayGPUSamples = _displayGPUSamples;
    os_unfair_lock_unlock(&_stateLock);
    FPEnqueueTelemetryRecord(telemetry, runtimeCounters);
    atomic_store(&_telemetryWritePending, false);
}
...
- (void)failWithReason:(const char *)reason
{
    [self failWithReason:reason expectedSurfaceGeneration:0];
}
    ...
    expectedSurfaceGeneration:(uint64_t)expectedSurfaceGeneration
{
    ...
    if (_state == NULL || atomic_load(&_invalidated) ||
        (expectedSurfaceGeneration &&
         expectedSurfaceGeneration != _surfaceGeneration) ||
        atomic_exchange(&_failed, true))
    {
        os_unfair_lock_unlock(&_stateLock);
        return;
    }
...
{
    if (!sourceDrawable || !sourceCommandBuffer) return 0;
    /* Avoid probing foreign objects or installing hooks when no session has
     * armed capture. Backend selection alone never activates this adapter. */
    BOOL captureActive = NO;
    os_unfair_lock_lock(&FPFrameGenerationSourceSessionsLock);
    for (FPFrameGenerationSourceSessionNode *node =
            FPFrameGenerationSourceSessions; node; node = node->_next)
    {
        if ([node->_session sourceCaptureIsEnabled])
        {
            captureActive = YES;
            break;
        }
    ...
    os_unfair_lock_unlock(&FPFrameGenerationSourceSessionsLock);
    if (!captureActive) return 0;
    id<CAMetalDrawable> drawable = (__bridge id<CAMetalDrawable>)sourceDrawable;
    id<MTLCommandBuffer> commandBuffer = (__bridge id<MTLCommandBuffer>)sourceCommandBuffer;
    if (![drawable conformsToProtocol:@protocol(CAMetalDrawable)] ||
        ![commandBuffer conformsToProtocol:@protocol(MTLCommandBuffer)] ||
        [commandBuffer status] >= MTLCommandBufferStatusCommitted)
        return 0;
    /* The existing fixed ticket/binding ledgers retain exactly the same
     * resources as the public presentDrawable hooks. The original renderer
     * still owns its scheduled handler, commit call, and presentation timing. */
    return FPBindTrackedSourceDrawable(drawable, commandBuffer, YES) ? 1 : 0;
}

#include "WineD3DOpenGLFrameGeneration.inc"
