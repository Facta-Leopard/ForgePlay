...
{
    bool enabled;
    bool capturedSourceClock;
    uint64_t observedContentSequence;
    bool error;
    bool outputActive;
    ...
    uint64_t midpointDroppedCurrentAge;
    ...
    double currentOutputMaximumAge;
...
static void clear_generation_reservation(FPFrameGenerationState *state)
{
    ...
    state->generationCurrentReadyTime = 0.0;
}
...
static void clear_queued_paired_current(FPFrameGenerationState *state)
{
    ...
    state->pairedCurrentReadyTime = 0.0;
}
...
FPFrameGenerationState *FPFrameGenerationStateCreateForCapturedSource(
    bool enabled, uint32_t targetFrameRate
)
{
    FPFrameGenerationState *state = FPFrameGenerationStateCreate(enabled, targetFrameRate);
    if (state) state->capturedSourceClock = true;
    return state;
}
...
{
    if (!state || !state->enabled || state->error || state->capturedSourceClock) return 0;
    ++state->sourcePresentSeen;
    return state->sourcePresentSeen;
}

static FPFrameGenerationSourceObservation observe_source_content(
    FPFrameGenerationState *state,
    double sourcePresentedTime
)
{
    ...
    if (!state || !state->enabled || state->error ||
        !isfinite(sourcePresentedTime) || sourcePresentedTime <= 0.0)
        return observation;
    if (!state->capturedSourceClock) ++state->sourcePresentSeen;
    if (state->hasLastObservedSourcePresentedTime &&
        sourcePresentedTime <= state->lastObservedSourcePresentedTime +
            source_timeline_tolerance(state))
        return observation;
    if (!state->capturedSourceClock) ++state->sourcePresentAccepted;
    observation.accepted = true;
    observation.sourceSequence = ++state->observedContentSequence;
    if (state->hasLastObservedSourcePresentedTime)
    {
    ...
    if (!state->capturedSourceClock) (void)append_positive_cadence_sample(
        state,
        state->sourcePresentationSamples,
        &state->sourcePresentationSampleCount,
        &state->sourcePresentationSampleCursor,
...
FPFrameGenerationSourceObservation FPFrameGenerationObserveSourcePresent(
    FPFrameGenerationState *state, double sourcePresentedTime
)
{
    if (!state || state->capturedSourceClock)
        return (FPFrameGenerationSourceObservation){0};
    return observe_source_content(state, sourcePresentedTime);
}

FPFrameGenerationSourceObservation FPFrameGenerationObserveSourceCaptureAvailable(
    FPFrameGenerationState *state, double captureAvailableTime
)
{
    if (!state || !state->capturedSourceClock)
        return (FPFrameGenerationSourceObservation){0};
    return observe_source_content(state, captureAvailableTime);
}
...
{
    return FPFrameGenerationRecordCaptureReadyAtTime(
        state, sourceSequence, sourcePresentedTime, 0.0
    );
}

FPFrameGenerationCapturePlan FPFrameGenerationRecordCaptureReadyAtTime(
    FPFrameGenerationState *state,
    uint64_t sourceSequence,
    double sourcePresentedTime,
    double readyTime
)
{
    ...
    if (!state || !state->enabled || state->error ||
        ...
        !sourceSequence || sourceSequence > state->observedContentSequence ||
    ...
    if (!isfinite(readyTime) || readyTime < sourcePresentedTime)
        readyTime = 0.0;
    ++state->epoch;
    ...
    if (state->hasLastCapturedSourcePresentedTime)
    {
            ...
            interval <= FP_FRAME_GENERATION_CADENCE_WINDOW_SECONDS)
        {
                ...
                !state->readyPair && !state->pairedCurrentQueued)
            {
                ...
                state->generationCurrentReadyTime = readyTime;
...
{
    return FPFrameGenerationAcquireDisplayCandidateAtTime(state, 0.0);
}

FPFrameGenerationDisplayCandidate FPFrameGenerationAcquireDisplayCandidateAtTime(
    FPFrameGenerationState *state,
    double callbackTime
)
{
    ...
    else
    {
        ...
        /* Only ready queue residence competes with optional output service.
         * Source-present age also includes renderer completion and compositor
         * callbacks; counting that baseline can discard every fresh midpoint.
         * Do not infer queue age from a future target or an unknown ready time.
         * This also retires an old optional pair
         * while older accepted originals drain, without changing their FIFO. */
        dropForCurrentAge = state->readyPair &&
            isfinite(callbackTime) && callbackTime > 0.0 &&
            isfinite(state->generationCurrentReadyTime) &&
            state->generationCurrentReadyTime > 0.0 &&
            callbackTime - state->generationCurrentReadyTime +
                FP_THRESHOLD_TOLERANCE_SECONDS >= effective_display_slot(state);
        if (state->readyPair &&
            (dropTimelineReversal || stalePair || dropForServiceCapacity ||
             dropForHeadroom || dropForCurrentAge ||
             (!state->outputActive && !olderCurrentPrecedesPair)))
        {
            ...
            else if (dropForCurrentAge)
            {
                ++state->midpointDroppedLate;
                ++state->midpointDroppedCurrentAge;
            }
            ...
            copy_text(
                ...
                dropTimelineReversal ? "midpoint-dropped-timeline" :
                    (dropForCurrentAge ? "midpoint-dropped-current-age" :
        ...
        else if (state->readyPair)
        {
            ...
            state->pairedCurrentReadyTime = readyCurrent.readyTime;
            clear_ready_pair(state);
        }
...
)
{
    ...
    if (candidate.kind == FPFrameGenerationOutputCurrent)
    {
        if (isfinite(candidate.sourcePresentedTime) &&
            candidate.sourcePresentedTime > 0.0 &&
            presentedTime >= candidate.sourcePresentedTime)
            state->currentOutputMaximumAge = fmax(
                state->currentOutputMaximumAge,
                presentedTime - candidate.sourcePresentedTime
            );
...
FPFrameGenerationProgress FPFrameGenerationGetProgress(
    const FPFrameGenerationState *state
)
{
    FPFrameGenerationProgress progress = {0};
    if (!state) return progress;
    progress.captureDemandActive = state->enabled && !state->error &&
        (state->mode == FPFrameGenerationModePriming ||
         state->mode == FPFrameGenerationModeActive);
    progress.outputActive = state->outputActive;
    progress.presentationsInFlight = state->writersInFlight;
    progress.presentationReceiptsPending = state->outputJoinCount;
    progress.midpointPresented = state->midpointPresented;
    progress.effectiveDisplaySlotDuration = effective_display_slot(state);
    progress.readyMidpointSourceTime = state->readyPairMidpointSourcePresentedTime;
    return progress;
}
...
)
{
    ...
    telemetry.midpointDroppedTimelineReversal =
        state->midpointDroppedTimelineReversal;
    telemetry.midpointDroppedCurrentAge = state->midpointDroppedCurrentAge;
    ...
    telemetry.readyMidpointSourceTime =
        state->readyPairMidpointSourcePresentedTime;
    telemetry.currentQueueDepth = state->readyCurrentCount +
        (state->pairedCurrentQueued ? 1u : 0u);
    double oldestCurrentTime = state->pairedCurrentQueued
        ? state->pairedCurrentReadyTime : 0.0;
    for (uint32_t index = 0; index < state->readyCurrentCount; ++index)
    {
        double readyTime = state->readyCurrents[index].readyTime;
        if (isfinite(readyTime) && readyTime > 0.0 &&
            (oldestCurrentTime <= 0.0 || readyTime < oldestCurrentTime))
            oldestCurrentTime = readyTime;
    }
    if (isfinite(oldestCurrentTime) && oldestCurrentTime > 0.0 &&
        observationTime >= oldestCurrentTime)
        telemetry.currentQueueOldestAge = observationTime - oldestCurrentTime;
    telemetry.currentOutputMaximumAge = state->currentOutputMaximumAge;
