...
# New packages authenticate the complete builtin graphics boundary. Older
# packages keep their original core map and cannot claim WineD3D availability.
WINED3D_PATHS = tuple(
    f"wine/lib/wine/{architecture}/{module}.dll"
    for architecture in ("i386-windows", "x86_64-windows")
    for module in (
        "ddraw", "d3d8", "d3d9", "d3d10", "d3d10_1", "d3d10core",
        "d3d11", "dxgi", "wined3d", "opengl32", "win32u",
    )
) + (
    "wine/lib/wine/x86_64-unix/opengl32.so",
...
def declared_wined3d_group(manifest: dict[str, object]) -> bool:
    payloads = manifest.get("corePayloadSHA256")
    if not isinstance(payloads, dict):
        fail("runtime core payload map is invalid")
    paths = set(payloads)
    if paths == set(REQUIRED_PATHS):
        return False
    if paths == set(REQUIRED_PATHS) | set(WINED3D_PATHS):
        return True
    fail("runtime core payload paths are incomplete or unsupported")


def generate(runtime_root: Path, include_wined3d: bool | None = None) -> dict[str, object]:
    root = runtime_root.resolve(strict=True)
    if include_wined3d is None:
        manifest_path = root / "RuntimeManifest.json"
        include_wined3d = declared_wined3d_group(load_manifest(manifest_path)) if manifest_path.exists() else False
    paths = (*REQUIRED_PATHS, *WINED3D_PATHS) if include_wined3d else REQUIRED_PATHS
    payloads = {
...
def verify(runtime_root: Path, manifest_path: Path) -> None:
    ...
    actual = generate(runtime_root, include_wined3d=declared_wined3d_group(expected))
...
def main(arguments: list[str]) -> int:
    if len(arguments) == 4 and arguments[1] == "generate" and arguments[3] == "--wined3d":
        print(json.dumps(generate(Path(arguments[2]), include_wined3d=True), sort_keys=True, separators=(",", ":")))
        return 0
    ...
    if len(arguments) == 4 and arguments[1] == "verify":
        ...
        return 0
    print(
        f"usage: {arguments[0]} generate <runtime-root> [--wined3d] | "
