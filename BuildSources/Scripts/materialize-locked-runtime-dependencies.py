...
def load_lock(path: Path) -> tuple[list[object], list[object]]:
    ...
    if payload["provider"] not in {
        ...
        fail("runtime dependency lock provider is unsupported")
...
def validate_artifact(raw: object) -> dict[str, object]:
    if not isinstance(raw, dict) or set(raw) - {"sourceKind"} != {
        "formula",
        "formulaVersion",
        "licenseExpression",
        "licensePaths",
    ...
    source_kind = raw.get("sourceKind", "homebrew-core-prebuilt-package")
    if source_kind == "homebrew-core-prebuilt-package":
        expected_reference = "https://formulae.brew.sh/formula/"
    elif source_kind == "forgeplay-source-build" and raw["formula"] == "molten-vk":
        expected_reference = "https://github.com/KhronosGroup/MoltenVK/tree/"
    else:
        fail("runtime dependency artifact sourceKind is unsupported")
    if not isinstance(raw["sourceReference"], str) or not raw["sourceReference"].startswith(expected_reference):
        fail("runtime dependency artifact sourceReference does not match its sourceKind")
...
def resolve_locked_source(
    ...
    requested_formula_root = package_prefix / "Cellar" / formula / version
    formula_root = require_directory(
        requested_formula_root,
        f"locked upstream formula {formula} {version}",
    ).resolve(strict=True)
    if formula_root != requested_formula_root:
        fail(f"locked upstream formula path must not traverse symlinks: {requested_formula_root}")
...
def main() -> int:
    ...
    try:
        ...
        requested_prefix = Path(sys.argv[2])
        package_prefix = require_directory(
            requested_prefix, "x86_64 prebuilt package prefix"
        ).resolve(strict=True)
        staging_root = require_directory(Path(sys.argv[3]), "staged runtime root").resolve(strict=True)
        if not requested_prefix.is_absolute() or package_prefix != requested_prefix:
            fail("locked x86_64 prebuilt package prefix must be canonical and must not traverse symlinks")
