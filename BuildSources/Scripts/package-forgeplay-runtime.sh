...
# Packaging bytes and build tools have different owners. A preserved, locked
# dependency prefix need not contain MinGW or pkg-config executables.
RUNTIME_DEPENDENCY_PREFIX="${FORGEPLAY_RUNTIME_DEPENDENCY_PREFIX:-$HOMEBREW_X86_PREFIX}"
...
EXPECTED_WINE_PATCH_SET_SHA256="a2a942633216cac183b1cb95c770d2ec05e10fd183244588b94a64e590ba05d6"
...
FORGEPLAY_WINE_MODIFICATIONS_SHA256="df1885d4ed532f9e091d11ce8bd6bb29685117e88afa6216fc1e75825c7385bc"
...
create_runtime_patch_projection() {
  ...
  if [[ "$MODE" == "public-source-package" ]]; then
    # Public source was authenticated before this function. Keep that export
    # pristine; the already-validated trusted workspace owns build scratch.
    workspace_root="$TRUSTED_GIT_REPOSITORY_INPUT"
  fi
  local temp_input="$workspace_root/Artifacts/Builds/Temporary"
  local workspace_directory
  for workspace_directory in "$workspace_root/Artifacts" "$workspace_root/Artifacts/Builds" "$temp_input"; do
    [[ ! -L "$workspace_directory" && ( ! -e "$workspace_directory" || -d "$workspace_directory" ) ]] ||
      fail "workspace temporary directory is unsafe: $workspace_directory"
    /bin/mkdir -p "$workspace_directory"
  done
  ...
  /bin/mkdir -m 700 "$PATCH_PROJECTION_WORKSPACE/tmp"
  export TMPDIR="$PATCH_PROJECTION_WORKSPACE/tmp"
...
def write_projection_file(path, payload):
    descriptor = os.open(
        path,
        os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_CLOEXEC | os.O_NOFOLLOW,
        0o600,
...
def write_order(path, names):
    write_projection_file(path, "".join(f"{name}\n" for name in names).encode("ascii"))
...
if not all(isinstance(value, list) for value in (patches, sidecars, contracts)):
    raise SystemExit("runtime patch projection inputs must contain arrays")
if any(not isinstance(entry, dict) for entry in patches):
    raise SystemExit("runtime patch entry must be an object")
# Build from the reviewed source order; never relabel the old packaged binary
# with patches that have not yet been compiled into it.
order = [entry.get("path") for entry in patches]
...
write_projection_file(
    manifest_snapshot,
    (json.dumps({"patchApplicationOrder": patch_names}, indent=2, sort_keys=True) + "\n").encode("ascii"),
)
PY
...
copy_renderer_license_files() {
  ...
  # The locked renderer tree may include dependency notices whose upstream
  # names do not start with LICENSE/COPYING/NOTICE (e.g. GCC exceptions).
  if [[ -d "$renderer_source/Legal" && ! -L "$renderer_source/Legal" ]]; then
    while IFS= read -r -d '' notice; do
      destination="$target/$(basename "$notice")"
      if [[ -e "$destination" ]]; then
        /usr/bin/cmp -s "$notice" "$destination" ||
          fail "renderer notice basename has conflicting contents: $destination"
      else
        /bin/cp "$notice" "$destination"
      fi
    done < <(/usr/bin/find "$renderer_source/Legal" -maxdepth 1 -type f -print0)
  fi
}
...
materialize_locked_renderer_payload() {
  ...
  /usr/bin/python3 "$RENDERER_PAYLOAD_MATERIALIZER" \
    ...
    "$target"
  local renderer
  for renderer in d3dmetal dxmt dxvk; do
...
verify_locked_renderer_payload() {
  ...
  for required_renderer in d3dmetal dxmt dxvk; do
  ...
  [[ ! -e "$renderer_root/d9vk" ]] ||
    fail "retired D9VK must not be included in the current renderer payload"
  local builtin_arch builtin_module
  for builtin_arch in x86_64-windows i386-windows; do
    for builtin_module in ddraw d3d8 d3d9 d3d10 d3d10_1 d3d10core d3d11 dxgi wined3d opengl32 win32u; do
      require_staged_renderer_file "$STAGING/wine/lib/wine/$builtin_arch/$builtin_module.dll" \
        "WineD3D $builtin_arch builtin $builtin_module"
    done
  done
  for builtin_module in opengl32 win32u winemac; do
    require_staged_renderer_file "$STAGING/wine/lib/wine/x86_64-unix/$builtin_module.so" \
      "WineD3D native $builtin_module"
  done
  ...
  if [[ -d "$dxvk_root" ]]; then
    local arch dll
    # Both architectures were exercised with the Vulkan WineD3D query path.
    for arch in x86_64-windows i386-windows; do
      for dll in d3d10core.dll d3d11.dll; do
        require_staged_renderer_file "$dxvk_root/wine/$arch/$dll" "DXVK $arch Direct3D 10/11 $dll"
      done
      for dll in d3d10.dll d3d10_1.dll dxgi.dll; do
        require_staged_renderer_file "$STAGING/wine/lib/wine/$arch/$dll" "DXVK bundled Wine $arch builtin $dll support"
      done
      for dll in d3d8.dll d3d9.dll d3d10core.dll d3d11.dll dxgi.dll; do
        case "$arch/$dll" in
          x86_64-windows/d3d10core.dll|x86_64-windows/d3d11.dll|i386-windows/d3d10core.dll|i386-windows/d3d11.dll) continue ;;
        esac
        [[ ! -e "$dxvk_root/wine/$arch/$dll" && ! -L "$dxvk_root/wine/$arch/$dll" ]] ||
          fail "DXVK macOS Direct3D 10/11 profile must not contain $arch/$dll"
      done
    done
  fi
}
...
# Bind the notice to this newly built payload, not the historical runtime used
# as the legal-text template. Preserve the licensing section verbatim.
/usr/bin/python3 - "$STAGING/Legal/Wine/FORGEPLAY-MODIFICATIONS.md" \
  "$WINE_SOURCE_TREE_SHA256" "$WINE_PATCH_SET_SHA256" "${#RUNTIME_PATCH_ORDER[@]}" <<'PY'
import re
import sys
from pathlib import Path

path = Path(sys.argv[1])
source, patches, count = sys.argv[2:]
text = path.read_text(encoding="utf-8")
for pattern, replacement in (
    (r"(?m)^- Ordered patch count: \d+", "- Ordered patch count: " + count),
    (r"(- Patch-set SHA-256:\s*`)\w+(`)", lambda match: match[1] + patches + match[2]),
    (r"(- Patched source-tree SHA-256:\s*`)\w+(`)", lambda match: match[1] + source + match[2]),
):
    text, replaced = re.subn(pattern, replacement, text)
    if replaced != 1:
        raise SystemExit("Wine modification notice snapshot field is missing or ambiguous")
...
CORE_IDENTITY_JSON="$(/usr/bin/python3 "$RUNTIME_CORE_IDENTITY_TOOL" generate "$STAGING" --wined3d)" ||
...
- Wine attribution and modification licensing: source validation requires Wine's \`LICENSE\`, \`COPYING.LIB\`, and \`AUTHORS\` files, and packaging copies all three into \`Legal/Wine\`. The tracked \`Legal/Wine/FORGEPLAY-MODIFICATIONS.md\` records the exact packaged patch identity and the current LGPL/GPL boundary; the Game Mode legal directory carries both unmodified license texts and the exact conversion scope.
...
- Steam game renderer policy: the current app exposes WineD3D, D3DMetal-NVIDIA, DXMT, and DXVK. The plain D3DMetal selector and old D9VK values remain readable only for legacy configuration restoration. WineD3D uses its bundled Vulkan renderer through WineVulkan and MoltenVK. The verified target is 32-bit Direct3D 9 / Shader Model 3; 64-bit compatibility and Direct3D 10/11/12 support are not claimed for this profile. Its optional frame generation and Frame Check default to off. The verified macOS DXVK profile provides 32-bit and 64-bit Direct3D 10/11 through its d3d10core/d3d11 modules and the bundled Wine builtin d3d10/d3d10_1/dxgi support; it does not advertise Direct3D 8/9 rendering. Both D3DMetal selectors address the same D3DMetal renderer payload. The NVIDIA selector projects one complete RTX 4090 / 561.09 identity tuple only to the Steam session control plane and routed game descendants; D3DMetal, normal in-game NVAPI/NGX calls, and Wine's primary-GPU and SetupAPI/Display Class/DirectX writers consume that same tuple, including Windows Display/UMD version \`32.0.15.6109\`. DXGI user-mode driver version checks follow the public contract: \`CheckInterfaceSupport(IDXGIDevice)\` is the version query, while Direct3D 11-or-later interface IIDs remain unsupported. Neither selector forces DirectX 11/12 or guarantees that a game exposes DLSS. Patched i386/x86_64 \`kernelbase.dll\` applies only the selected renderer to Steam game children for the session. Direct3D import classification, loader-stage profiles, and mixed renderer compositions are disabled. Native renderer policies expose only their selected architecture-specific external root. The explicit WineD3D policy keeps renderer activation and lineage enabled while using the bundled Wine DLL path, builtin DLL preferences, and \`WINE_D3D_CONFIG=renderer=vulkan\`; only this policy permits empty external component and DLL-directory controls. Route V2 records the plan. Native Load V3 records use \`path-owner=verified\` for an exact external allowlist match; WineD3D records use \`path-owner=builtin\` for a Wine builtin image, with package and prefix identity validation remaining separate. If Wine cannot construct the selected renderer DLL search path, it records the loader error and continues on Wine's default DLL path instead of terminating the game process; the host keeps that degradation diagnostic and never reports it as successful NVIDIA/frame-generation activation.
...
- Renderer payload: the three locked Apple GPTK/D3DMetal, DXMT, and DXVK component trees are verified from the explicit build-time \`FORGEPLAY_RENDERER_SOURCE\`, then bundled under \`Frameworks/renderer\`. WineD3D uses the authenticated Wine builtin module closure plus the existing WineVulkan and MoltenVK libraries; no external WineD3D or D9VK payload is installed. The installed app has no external runtime dependency. ForgePlay derives only the exact \`nvapi.dll\` filename copy and \`nvapi.so\` internal link after upstream-tree verification, validates both independently, and identifies them as derived aliases in \`RuntimeSBOM.json\`.
...
for patch_file in \
  ...
  Patches/wine-11.12-media-foundation-video-output-negotiation.patch \
  Patches/wine-11.12-forgeplay-xbox-iohid-normalization.patch \
  Patches/wine-11.12-forgeplay-frame-generation-renderer-routing.patch; do
...
renderer environment. The current app exposes WineD3D, D3DMetal-NVIDIA, DXMT, and DXVK;
plain D3DMetal remains recognized as a hidden legacy selector. Old D9VK values remain
readable only to restore legacy configuration safely. WineD3D uses its bundled Vulkan backend
through WineVulkan and MoltenVK. Its verified target is 32-bit Direct3D 9 / Shader Model 3;
64-bit compatibility and Direct3D 10/11/12 support are not claimed for this profile. Optional frame generation and Frame Check default to off. The verified macOS
DXVK profile supplies 32-bit and 64-bit Direct3D 10/11 through d3d10core/d3d11 and the bundled Wine builtin
d3d10/d3d10_1/dxgi support. Direct3D 8/9 DXVK rendering is not advertised. Both D3DMetal selectors
...
are not used. For native renderer policies, the Unix loader places only the selected renderer root
ahead of Wine's compiled DLL directory, while the Windows loader prepends only its matching i386 or
x86_64 directories. The explicit \`wined3d\` policy retains active renderer and lineage markers,
selects builtin ddraw/d3d8/d3d9/d3d10/d3d10_1/d3d10core/d3d11/dxgi/wined3d/opengl32 modules,
and uses \`WINE_D3D_CONFIG=renderer=vulkan\` without an external component or DLL directory.
Nonempty external controls are rejected for that builtin policy. If that
...
records use \`manual-session-d3dmetal\`, \`manual-session-dxmt\`, \`manual-session-wined3d-vulkan\`, or
  \`manual-session-dxvk\` as the exact selection reason and describe the selected plan. A Load V3
  record identifies an external renderer path match only when its resolved path exactly matches
  the active architecture-specific allowlist and reports \`path-owner=verified\`. For WineD3D,
  \`path-owner=builtin\` identifies a Wine builtin image; it does not bypass package or prefix
  hash ownership checks. Optional frame generation uses the existing Vulkan-to-Metal presentation
  path and its source-command-buffer and actual current/generated presentation receipts.
  Wine OpenGL libraries remain internal dependencies; this profile selects Vulkan rendering. Renderer state remains process-scoped and is scrubbed from Steam
...
are copied from the same pinned input versions into \`Legal/\`.

MoltenVK 1.4.1 includes ForgePlay's optional pre-commit source-drawable binding.
Its source-built artifact is distinguished from the unchanged Homebrew payloads
in the dependency lock and Runtime SBOM. The original scheduled presentation
remains intact. The exact source revisions, patch, build record, modification
notice and dependency licenses are packaged under \`Legal/MoltenVK/SourceBuild/\`.
The maintained build recipe and source lock are \`Scripts/build-forgeplay-moltenvk.sh\`
and \`Config/ForgePlayMoltenVKSource.lock.json\` in the project source.
