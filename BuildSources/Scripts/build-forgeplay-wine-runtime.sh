...
current_binary_manifest, lock_path, patch_root = map(Path, sys.argv[1:])
lock = json.loads(lock_path.read_text(encoding="utf-8"))
locked = lock.get("patches")
if not isinstance(locked, list) or not locked or any(not isinstance(entry, dict) for entry in locked):
    raise SystemExit("runtime patch provenance inventory is malformed")
# The reviewed source inputs own this build. The packaged manifest continues
# to describe the previous binary until a newly built package replaces it.
order = [entry.get("path") for entry in locked]
if len(order) != len(set(order)):
    raise SystemExit("runtime provenance-locked patch order must be unique")
for entry in locked:
    name = entry["path"]
...
workspace_builds = os.path.join(repository, "Artifacts", "Builds")
for label, output in (("build", build), ("install", install)):
    common = os.path.commonpath([output, repository])
    if common == output:
        raise SystemExit(f"{label} root must not equal or contain the repository")
    if common == repository and (
        output == workspace_builds
        or os.path.commonpath([output, workspace_builds]) != workspace_builds
    ):
        raise SystemExit(f"workspace-local {label} root must be below Artifacts/Builds")
PY
...
/bin/mkdir -m 700 "$BUILD_ROOT/tmp"
