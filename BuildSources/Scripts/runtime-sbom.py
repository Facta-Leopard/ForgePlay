...
REVIEWED_SOURCE_BUILD_POLICY = "official-or-reviewed-source-build-v2"
RENDERER_PROVENANCE_POLICIES = {
    "official-or-upstream-artifact-only-v1",
    REVIEWED_SOURCE_BUILD_POLICY,
}
D9VK_SOURCE_BUILD_LICENSE_PATHS = {
    "Legal/d9vk/NOTICE-ForgePlay.md",
    "Legal/d9vk/SourceBuildRecord.json",
    "Legal/d9vk/ForgePlayD9VKSource.lock.json",
}
...
def dependency_provenance(lock_path: Path) -> dict[str, dict[str, object]]:
    ...
    if (
        ...
        or payload.get("provider") not in {
            "homebrew-core-prebuilt-package", "forgeplay-curated-runtime-dependencies"
        }
    ):
    ...
    for artifact in artifacts:
        if not isinstance(artifact, dict) or set(artifact) - {"sourceKind"} != required_fields:
        ...
        source_kind = artifact.get("sourceKind", "homebrew-core-prebuilt-package")
        if source_kind == "homebrew-core-prebuilt-package":
            expected_reference = "https://formulae.brew.sh/formula/"
        elif source_kind == "forgeplay-source-build" and artifact["formula"] == "molten-vk":
            expected_reference = "https://github.com/KhronosGroup/MoltenVK/tree/"
        else:
            fail(f"runtime dependency artifact has an unsupported source kind: {target_path}")
        if not artifact["sourceReference"].startswith(expected_reference):
...
def renderer_contracts(lock_path: Path) -> dict[str, dict[str, object]]:
    ...
    if (
        ...
        or not isinstance(payload["provenancePolicy"], str)
        or payload["provenancePolicy"] not in RENDERER_PROVENANCE_POLICIES
    ):
    ...
    for component in components:
        ...
        reviewed_source_build = (
            payload["provenancePolicy"] == REVIEWED_SOURCE_BUILD_POLICY
            and name == "d9vk"
            and component["sourceKind"] == "forgeplay-source-build"
        )
        if component["sourceKind"] != RENDERER_SOURCE_KINDS[name] and not reviewed_source_build:
            fail(f"renderer payload sourceKind violates the provenance policy: {name}")
        ...
        if reviewed_source_build and not D9VK_SOURCE_BUILD_LICENSE_PATHS.issubset(license_paths):
            fail("reviewed d9vk source build must declare its notice, build record, and source lock")
    ...
    if set(mapped) not in (RENDERER_COMPONENTS, {"d3dmetal", "dxmt", "dxvk"}):
...
def verify_reviewed_renderer_source_records(component_root: Path, contract: dict[str, object]) -> None:
    if contract["name"] != "d9vk" or contract["sourceKind"] != "forgeplay-source-build":
        return
    # The packager copies component/Legal into runtime Legal/d9vk. Require the
    # review records inside the component too, so its pinned tree covers them.
    require_directory(component_root / "Legal", "reviewed d9vk source build records")
    for license_path in sorted(D9VK_SOURCE_BUILD_LICENSE_PATHS):
        require_regular_file(
            component_root / "Legal" / PurePosixPath(license_path).name,
            "reviewed d9vk source build record",
        )
...
def verify_renderer_payload(
    ...
    for name, contract in contracts.items():
        ...
        verify_reviewed_renderer_source_records(component_root, contract)
...
def create_sbom(
    ...
    return {
        ...
        "policy": {
            ...
            "rendererProvenance": (
                "official-or-reviewed-source-build-with-verified-local-aliases-v2"
                if any(contract["sourceKind"] == "forgeplay-source-build" for contract in renderer_map.values())
                else "official-or-upstream-artifact-with-verified-local-aliases-v1"
            ),
