...
def load_lock(path: Path) -> dict[str, dict[str, object]]:
    ...
    for component in components:
        ...
        if reviewed_source_build and not D9VK_SOURCE_BUILD_LICENSE_PATHS.issubset(component["licensePaths"]):
    ...
    if set(mapped) not in (set(RENDERER_SOURCE_KINDS), {"d3dmetal", "dxmt", "dxvk"}):
...
def verify_component(path: Path, contract: dict[str, object], sbom_helpers) -> None:
    ...
    try:
        sbom_helpers.verify_reviewed_renderer_source_records(path, contract)
